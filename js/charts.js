...
export function renderTrend(rows,state,extent){
  ...
  for(const [mode,color,dash] of modes){
    ...
    data.forEach((d,i)=>{svg+=`<circle cx="${x(i)}" cy="${y(d[mode])}" r="4.5" fill="var(--surface)" stroke="${color}" stroke-width="2.5"><title>${d.year}${d.partial?' (partial)':''}: ${number(d[mode])} ${mode}-involved crashes</title></circle>`;});
  }
